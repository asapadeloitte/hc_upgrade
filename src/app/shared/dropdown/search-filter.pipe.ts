...
export class MultiSelectSearchFilter implements PipeTransform {
  ...
  private _lastOptions: ISelectOption[];
  private _searchCache: { [k: string]: ISelectOption[] } = {};
  private _searchCacheInclusive: { [k: string]: boolean | number } = {};

  transform(options: Array<ISelectOption>, str: string, limit = 0, renderLimit = 0): Array<ISelectOption> {
    str = (str || '').toLowerCase();
    ...
    const matchPredicate = (option: ISelectOption) => regexp.test(option.name);
    const getChildren = (option: ISelectOption) => options.filter(child => child.parentId === option.id);
    const getParent = (option: ISelectOption) => options.find(parent => option.parentId === parent.id);
